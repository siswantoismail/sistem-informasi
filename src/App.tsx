...
const API_URL = "https://sistem-informasi-production-432f.up.railway.app";
...
export default function App() {
  ...
  const fetchStats = async () => {
    try {
      const res = await fetch(`${API_URL}/api/gtk/stats/summary`);
      if (res.ok) {
        const data = await res.json();
        setStats(data);
      }
  ...
  const fetchGTKData = useCallback(
    async (q = searchTerm, tipe = selectedTipe, status = selectedStatus) => {
      setIsLoading(true);
      try {
        ...
        const url = `${API_URL}/api/gtk?${params.toString()}`;
  ...
  const handleRefresh = async () => {
    await Promise.all([
      fetchDbStatus(),
      fetchStats(),
      fetchGTKData(searchTerm, selectedTipe, selectedStatus),
    ]);
  ...
  const handleFormSubmit = async (formData: GTKFormData) => {
    ...
    try {
      if (editingItem) {
        // Update
        const res = await fetch(`${API_URL}/api/gtk/${editingItem.id}`, {
          method: "PUT",
        ...
        const res = await fetch(`${API_URL}/api/gtk`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(formData),
        });
  ...
  const handleDeleteConfirm = async (id: number) => {
    setIsSubmitting(true);
    try {
      const res = await fetch(`${API_URL}/api/gtk/${id}`, {
        method: "DELETE",
      });
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `data_gtk_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const filteredList =
    selectedAktif !== "Semua"
      ? gtkList.filter((item) => item.status_aktif === selectedAktif)
      : gtkList;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col antialiased">
      ...
      <footer className="bg-white border-t border-slate-200 mt-auto py-6 text-center text-xs text-slate-500">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p>
            © {new Date().getFullYear()} SIM GTK - Sistem Informasi Guru &amp;
            Tenaga Kependidikan
          </p>
