...
async function startServer() {
  ...
  app.get('/api/gtk/stats/summary', async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ error: err.message || 'Gagal mengambil ringkasan statistik' });
    }
  });
  ...
  app.get('/api/gtk', async (req: Request, res: Response) => {
    try {
      ...
      const tipe = typeof req.query.tipe === 'string' ? req.query.tipe : undefined;
      const status = typeof req.query.status === 'string' ? req.query.status : undefined;

      const items = await getAllGTK({ q, tipe, status });
      res.json(items);
    } catch (err: any) {
      res.status(500).json({ error: err.message || 'Gagal mengambil data GTK' });
    }
  });
  ...
  app.get('/api/gtk/:id', async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ error: err.message || 'Gagal mengambil detail GTK' });
    }
  });
  ...
  app.post('/api/gtk', async (req: Request, res: Response) => {
    try {
      ...
      if (!nama || !jenis_kelamin || !tipe || !jabatan || !status_kepegawaian || !pendidikan_terakhir || !jurusan || !email || !telepon || !alamat) {
        return res.status(400).json({
          error: 'Semua kolom wajib (Nama, Jenis Kelamin, Tipe, Jabatan, Status Kepegawaian, Pendidikan, Jurusan, Email, Telepon, Alamat) harus diisi.',
        });
      }
      ...
      res.status(500).json({ error: err.message || 'Gagal menambahkan data GTK' });
    }
  });
  ...
  app.put('/api/gtk/:id', async (req: Request, res: Response) => {
    try {
      ...
      if (!existing) {
        return res.status(404).json({ error: 'Data GTK yang ingin diubah tidak ditemukan' });
      }
  ...
  app.delete('/api/gtk/:id', async (req: Request, res: Response) => {
    try {
      ...
      const existing = await getGTKById(id);
      if (!existing) {
        return res.status(404).json({ error: 'Data GTK yang ingin dihapus tidak ditemukan' });
      }
      ...
      res.status(500).json({ error: err.message || 'Gagal menghapus data GTK' });
    }
  });
